...
if(isset($_SESSION['y3adi']) && isset($_SESSION['valide_token_donneur']))
{
	...
	}else
	{
		if($row['blastek']==="h_bancsang_maalem_d" || $row['blastek']==="h_bancsang_d")
		{
            ...
            $get_locale=mysqli_query($connection,"SELECT * FROM `enregistrer_donneur` WHERE `vsidentite`='{$row['vsidentite']}' AND `type_enregistrement`='disponible_d' AND `status_enregistrement`='valide_d'");
            if(mysqli_num_rows($get_locale) > 0)
            {
                ...
                while($data=mysqli_fetch_assoc($get_locale))
                {
                    ...
                    <tr>
                        ...
                        <td><a href="sup_don.php?v='.$data['url_enregistrement'].'" class="btn btn-danger">Supprimer</a></td>
