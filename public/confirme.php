	...
	}else
	{
		if($row['blastek']==="hopitale_b_d")
		{
                ...
                if(isset($_GET['v']) && !empty($_GET['v']))
                {
                    ...
                    $stmt_check_valid_token=$connection->prepare("SELECT * FROM `demmande_poches` WHERE `url_demmande`=? AND `url1`=? LIMIT 1");
                    ...
                    if(mysqli_num_rows($result)>0)
                    {
                            ...
                            $stmt_delete=$connection->prepare("DELETE FROM `demmande_poches` WHERE `url_demmande`=? AND `url1`=? LIMIT 1");
                            $stmt_delete->bind_param("ss",$fetch['url_demmande'],$row['vsidentite']);
                            $stmt_delete->execute();
                            ...
                            {   
                                ...
                                $_SESSION['creat']="Demmande Supprimeé Avec succssé";
