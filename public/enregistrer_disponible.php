...
if(!isset($_SESSION['ana_hwa']) || !isset($_POST['csrf_token']))
{
    ...
    redirect('enregistrerdonneurs_D.php');
}
        ...
        if(isset($_POST['enregistrer_d']))
        {
                ...
                if($error==0)
                {
                    ...
                    $url=bin2hex(random_bytes(32));
                        $insert_enreg=$connection->prepare("INSERT INTO `enregistrer_donneur`(`nom_donneur_1`, `prenom_donneur_1`, `age_donneur_1`, `groupage_donneur_1`, `type_enregistrement`, `date_enregistrement`, `nom_hopitale`, `ville_enregistrement`, `url_enregistrement`, `vsidentite`)
                         VALUES (?,?,?,?,'disponible_d',?,?,?,?,?)");
                         $insert_enreg->bind_param("ssssssssi",$nom_donneur,$prenom_donneur,$age_donneur,$groupage_donneur,$date,$row['nom_donneur_s'],$row['ville_donneur_s'],$url,$row['vsidentite']);
