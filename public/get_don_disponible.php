...
if(isset($_SESSION['y3adi']) && isset($_SESSION['valide_token_donneur']))
{
	...
	}else
	{
		if($row['blastek']==="h_bancsang_maalem_d" || $row['blastek']==="h_bancsang_d")
		{
            ...
            $get_locale=mysqli_query($connection,"SELECT * FROM `enregistrer_donneur` WHERE `vsidentite`='{$row['vsidentite']}' AND `type_enregistrement`='disponible_d' AND `status_enregistrement`='en attente_d'");
            if(mysqli_num_rows($get_locale) > 0)
            {
                ...
                while($data=mysqli_fetch_assoc($get_locale))
                {
                    ...
                    <tr>
                        ...
                        <td><a href="valide_disponible.php?v='.$data['url_enregistrement'].'" onclick="return confirm(\'Êtes-vous sûr de vouloir valider cet enregistrement ?\');" class="btn btn-success">Valider</a></td>
                        <td><a href="rejeter_disponible.php?v='.$data['url_enregistrement'].'" onclick="return confirm(\'Êtes-vous sûr de vouloir rejeter cet enregistrement ?\');" class="btn btn-danger">Rejeter</a></td>
                    </tr>';
