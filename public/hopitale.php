			...
			<div class="info-data">
				...
				<div class="card">
						<?php 
							...
							$not=mysqli_query($connection,"SELECT * FROM `enregistrer_donneur` WHERE `nom_hopitale`='{$row['nom_donneur_s']}' AND `ville_enregistrement`='{$row['ville_donneur_s']}' AND `status_enregistrement`='valide_d' AND `type_enregistrement`='hopitale_d'");
							$num_not=mysqli_num_rows($not);
					...
					<div class="head">
						<div>
							<h2>Notification des Dons <span class="text-danger"> <?= $num_not; ?> </span></h2><br>
							<h6>Les Notifications des dons pour les patients de l'hopital <span class="text-primary"> <?=$row['nom_donneur_s']?> </span></h6>
						</div>
						
					</div>
					<a class="btn btn-primary mt-3" href="nottification.php">Notifications des dons</a>
					
				</div>
				<div class="card">
				<?php 

					$not_demande=mysqli_query($connection,"SELECT d.*,b.nom_donneur_s FROM `demmande_poches` d JOIN bnadms b ON b.vsidentite=d.url2 WHERE `url1`='{$row['vsidentite']}' AND `status_d`='confirmer'");
					$num=mysqli_num_rows($not_demande);
				...
				?>
					<div class="head">
						<div>
						<h2>Notification des Demandes <span class="text-danger"> <?= $num ?> </span></h2><br>
							<h6>Les Notifications des Demandes de sang ou produits sanguins de l'hopital <span class="text-primary"> <?=$row['nom_donneur_s']?> </span></h6>
						</div>
						
					</div>
					<a class="btn btn-primary" href="nottification_demmande.php">Notifications des Demandes</a>
