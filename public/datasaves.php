...
if($_POST['csrf_tokens']===$_SESSION['ana_hwapass'])
{
    ...
    }else
    {
        ...
        if(isset($_POST['nv_pass']))
        {
            ...
            if($error==0)
            {
                $stmt = $connection->prepare("SELECT `password_D` FROM `bnadms` WHERE `email_donneur_s` = ? AND `valide_token_donneur`=? AND `blastek`='h_b_donneurs' LIMIT 1");
                ...
                if (password_verify($password, $old_password)) {
                    ...
                    {   
                        $stmt = $connection->prepare("UPDATE `bnadms` SET `password_D` = ? WHERE `email_donneur_s` = ? AND `password_D`= ? AND `valide_token_donneur`=? AND `blastek`='h_b_donneurs' LIMIT 1");
