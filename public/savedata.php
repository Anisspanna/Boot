    ...
    }else
    {
        ...
        if(isset($_POST['save']))
        {
                ...
                if(!preg_match($nbr,$telephone))
                {
                    ...
                    {       
                        ...
                        }else
                        {
                            ...
                            $stmtupdate_phone = $connection->prepare("UPDATE `bnadms` SET `telephone_donneur_s` = ? WHERE `email_donneur_s` = ? AND `valide_token_donneur`=? AND `blastek`='h_b_donneurs' LIMIT 1");
            ...
            if(isset($_POST['age'])  && !empty($_POST['age']))
            {
              ...
              }elseif($row['agesdonneur_s']!=$age)
              {
                $stmtupdate_age = $connection->prepare("UPDATE `bnadms` SET `agesdonneur_s` = ? WHERE `email_donneur_s` = ? AND `valide_token_donneur`=? AND `blastek`='h_b_donneurs' LIMIT 1");
                $stmtupdate_age->bind_param("sss", $age, $_SESSION['y3adi'],$_SESSION['valide_token_donneur']);
                $stmtupdate_age->execute();
                if($stmtupdate_age->affected_rows >0)
                {
                    $A=2;
                    $stmtupdate_age->close();
               ...
               $error =1 ;
                ...
                if(isset($_POST['ville']) && !empty($_POST['ville']))
                {
                  ...
                  if (!in_array($ville, ['Adrar', 'Chlef', 'Laghouat', 'Oum El Bouaghi', 'Batna', 'Bejaia', 'Biskra', 'Bechar', 'Blida', 'Bouira', 'Tamanrasset', 'Tebessa', 'Tlemcen', 'Tiaret', 'Tizi Ouzou', 'Alger', 'Djelfa', 'Jijel', 'Setif', 'Saida', 'Skikda', 'Sidi Bel Abbes', 'Annaba', 'Guelma', 'Constantine', 'Medea', 'Mostaganem', 'MSila', 'Mascara', 'Ouargla', 'Oran', 'El Bayadh', 'Illizi', 'Bordj Bou Arreridj', 'Boumerdes', 'El Taref', 'Tindouf', 'Tissemsilt', 'El Oued', 'Khenchela', 'Souk Ahras', 'Tipaza', 'Mila', 'Ain Defla', 'Naama', 'Ain Témouchent', 'Ghardaia', 'Relizane', 'Timimoun', 'Bordj Badji Mokhtar', 'Ouled Djellal', 'Beni Abbes', 'In Salah', 'In Guezzam', 'Touggourt', 'Djanet', 'El Meghaier', 'El Menia'])) {
                    ...
                    $stmtupdate_ville = $connection->prepare("UPDATE `bnadms` SET `ville_donneur_s` = ? WHERE `email_donneur_s` = ? AND `valide_token_donneur`=? AND `blastek`='h_b_donneurs' LIMIT 1");
