		...
		if($row['blastek']==="h_bancsang_maalem_d" || $row['blastek']==="h_bancsang_d")
		{
                ...
                if(isset($_GET['v']) && !empty($_GET['v']))
                {
                    ...
                    if(mysqli_num_rows($result)>0)
                    {
                            ...
                            $stmt_update=$connection->prepare("UPDATE `enregistrer_donneur` SET `status_enregistrement`='valide_d' WHERE `url_enregistrement`=? AND `vsidentite`=? LIMIT 1");
                            $stmt_update->bind_param("ss",$fetch['url_enregistrement'],$row['vsidentite']);
                            $stmt_update->execute();
                            ...
                            {   
                                ...
                                $_SESSION['creat']="Le Don validé Avec succssé";
                                redirect('don_disponible.php');
                            }
