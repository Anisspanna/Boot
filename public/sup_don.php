	...
	}else
	{
		if($row['blastek']==="h_bancsang_maalem_d" || $row['blastek']==="h_bancsang_d")
		{
                ...
                if(isset($_GET['v']) && !empty($_GET['v']))
                {
                    ...
                    $stmt_check_valid_token=$connection->prepare("SELECT * FROM `enregistrer_donneur` WHERE `vsidentite`='{$row['vsidentite']}' AND `url_enregistrement`=? AND `type_enregistrement`='disponible_d' AND `status_enregistrement`='valide_d' LIMIT 1");
                    ...
                    if(mysqli_num_rows($result)>0)
                    {
                            ...
                            $stmt_delete=$connection->prepare("DELETE FROM `enregistrer_donneur` WHERE `url_enregistrement`=? AND `vsidentite`='{$row['vsidentite']}' AND `type_enregistrement`='disponible_d' AND `status_enregistrement`='valide_d' LIMIT 1");
                            $stmt_delete->bind_param("s",$fetch['url_enregistrement']);
                            $stmt_delete->execute();
                            ...
                            {   
                                ...
                                $_SESSION['creat']="Don supprimer succseé";
